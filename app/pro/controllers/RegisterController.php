...
class RegisterController extends BaseController {
    ...
    public function indexAction() {
        if($this->request->isPost()){
            ...
            if($register->create($this->request->getPost())){
                ...
                //$this->response->redirect("pro/register/confirm?task=".uniqid('xLmR'));
            }
            else{
                $this->flash->error(implode(",", $register->getMessages()));
                $this->response->redirect("pro/register?task=".uniqid('xLmR'));
            }
        }
    ...
    public function confirmAction(){
        ...
        return $this->response->redirect("pro/register?task=");
    }
    ...
    public function passwordAction() {
        ...
        if(!empty($hash) && !empty($email)) {
                ...
                if($password == $rpassword){
                    ...
                    if($register){
                        ...
                        if($updateRow){
                            ...
                            return $this->response->redirect('pro/');
                        }
                        else{
                            $this->flash->error("Unable to Update Password");
                            return $this->response->redirect('pro/register/password?hash='.$hash.'&email='.$email);
                        }
                    }
                    else{
            ...
            $this->response->redirect("pro/index?task=MISSING_HASH&emailTag");
        }
    }
}