...
use Multiple\Pro\Models\Profile;
        ...
        if($this->request->isPost()){
            ...
            if($this->request->hasPost("createBtn")) {
                if ($profile->create($argsRow)) {
                    ...
                    return $this->response->redirect("pro/settings?task=success");
                }
                else {
                    $this->flash->error(implode(", ", $profile->getMessages()));
                    return $this->response->redirect("pro/settings?task=failed");
                }
            }
        ...
        if($this->request->isPost()){
            ...
            if($register != false){
                ...
                if($register->update(['password' => $password])){
                    ...
                    $this->response->redirect("pro/settings/password?task=successful");
                }
                else{
                    $this->flash->success("Unable to update password");
                    $this->response->redirect("pro/settings/password?task=failure");
                }
            }
        }
        return $this->view->setRenderLevel(View::LEVEL_AFTER_TEMPLATE);
